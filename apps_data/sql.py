...
class MarriadbClient:
    ...
    """
    def __init__(self, host=None, user=None, password=None, charset="utf8mb4", local_infile=0, db=None, logger=None):
        # db settings
        self.host = host
        self.user = user
    ...
    def __del__(self):
        ...
        try:
            ...
            self.has_cursor = False
            self.logger.info(STR_CONST["db"]["CursorClose"])
        try:
            self.connection.close()
        finally:
            self.has_db_connection = False
            self.logger.info(STR_CONST["db"]["ConnectionClose"])
    ...
    def connect(self):
        """connect to database
        """
        self.logger.info(STR_CONST["db"]["ConnectionStart"])
        self.logger.info("host: {0}. database: {1}".format(self.host, self.db))
        try:
            ...
            self.logger.info(STR_CONST["db"]["ConnectionSuccess"])
            self.has_db_connection = True
        except Exception as e:
            self.logger.error(e)
            self.logger.info(STR_CONST["db"]["ConnectionError"])
            self.connection = None
